...
public class HdfsTool {
     ...
     * @throws IOException
     */
    public int listFiles(String targetDir,ArrayList<String[]> dirList,ArrayList<String[]> fileList,DirectoryMetaInfo metaInfo) throws IOException{
		...
		for (int i=0; i < files.length; i++) {
			...
			if (files[i].isDirectory()){
				if(!hasExecutePermission(files[i]))continue;
			}else{
				if(!hasReadPermission(files[i]))continue;
			}
			
			String cols[] = new String[10];
			cols[0] = revertInvalidChar(files[i].getPath().getName());
			metaInfo.setFileNameMaxLength(cols[0].getBytes().length,cols[0].length());
			cols[5] = dateForm.format(new Date((files[i].getModificationTime())));
			if (!files[i].isDirectory()) {
				cols[1] = "file";
     ...
     * sort the file list
     * @param fileList
     * @param col : the name of column,N:Name,M:modificationTime,S:size
     * @param orderType : D:desc,A:asc 
     */
    public void sortFileList(ArrayList<String[]> fileList,String col,String orderType,ArrayList<String[]> sortedFileList){
    ...
    String getParentDir(){
        ...
        if (parent != null && parent.length() >= SysConfig.ROOT_DIR.length()){
        	...
        	if(parent.length() == 0)parent = Path.SEPARATOR;
        }else{
        	parent = "";
        }
    	return parent;
     ...
     * @throws IOException
     */
    public Object checkFile(String targetDir,String targetFileName) throws IOException{
    	Path targetFile = new Path(convertInvalidChar(SysConfig.ROOT_DIR + targetDir + Path.SEPARATOR + targetFileName));
    	...
    	if(!hasReadPermission(targetFileStatus)){
            log.info("PERMISSIONS: " + targetFile.toString() + " - " + " read denied");
    		return new Integer(-1);
    	}
     ...
     */
    public boolean copyFull(FileStatus targetFileStatus,ServletOutputStream ostream){
        	...
        	is = dfs.open(targetFileStatus.getPath());
       	    while( ( bytesToRead = is.read(buffer) ) != -1){
       	    	try{
       	    		...
       	    		log.info("write file " + targetFileStatus.getPath() + " be aborted \n"
       	    				+ "ClientAbortException:  java.net.SocketException: " + e.getCause().getMessage());
       	    		return true;
       	    	}
       	    }
        }catch(IOException e){        	
     ...
     * @return copy result(true or false)
     */
    public boolean includeFile(String targetDir,String targetFileName,JspWriter out){
    ...
    /**
     * Copy the contents of the specified file to the specified
     * output stream, and ensure that both streams are closed before returning
     * (even in the face of an exception).
     *
     * @param targetFileStatus The file status object
     * @param ostream The output stream to write to
     * @param range Start and of the range which will be copied
     * @return Exception which occurred during processing
     * @throws IOException 
     */
    public boolean copyRange(FileStatus targetFileStatus,ServletOutputStream ostream,Range range){
        FSDataInputStream is = null;
        int bytesToRead = 0;
       	    ...
       	    while( ( bytesToRead = is.read(buffer) ) != -1){
       	    	...
       	    	try{
	       	    	if(readedLength >= shouldReadLength){
	       	    		ostream.write(buffer, 0, bytesToRead - (int)(readedLength - shouldReadLength));       	    		
	       	    		break;
	       	    	}else{
	       	    		ostream.write(buffer, 0, bytesToRead);
	       	    	}
    ...
    boolean hasReadPermission(FileStatus fileStatus) {
    	...
    	if(permissions.getOtherAction().equals(FsAction.READ_WRITE)
    		...
    		||permissions.getOtherAction().equals(FsAction.ALL))hasRead = true;
   			...
   			&&(permissions.getUserAction().equals(FsAction.READ_WRITE)
   				...
   				||permissions.getUserAction().equals(FsAction.READ_EXECUTE)
   				||permissions.getUserAction().equals(FsAction.ALL)
   				)) hasRead = true;
   			...
   			&&(permissions.getGroupAction().equals(FsAction.READ_WRITE)
   				...
   				||permissions.getGroupAction().equals(FsAction.READ_EXECUTE)
   				||permissions.getGroupAction().equals(FsAction.ALL)   				
   				)) hasRead = true;
    	if(hasRead)
    		log.debug("PERMISSIONS: " + fileStatus.getPath().toString() + " - " + " read allowed for current process");
    	return hasRead;
    }
    ...
    boolean hasExecutePermission(FileStatus fileStatus) {
    	...
    	if(permissions.getOtherAction().equals(FsAction.READ_EXECUTE)
   			||permissions.getOtherAction().equals(FsAction.ALL) )hasExcute = true;
    	else if(userName.equals(fileStatus.getOwner())
    		&& (permissions.getUserAction().equals(FsAction.READ_EXECUTE)
    		||permissions.getUserAction().equals(FsAction.ALL)))hasExcute = true;
    	else if (isGroupMember(fileStatus.getGroup(),groupNames)
    		&& (permissions.getGroupAction().equals(FsAction.READ_EXECUTE)
    		||permissions.getGroupAction().equals(FsAction.ALL)))hasExcute = true;
    	if(hasExcute)
    		log.debug("PERMISSIONS: " + fileStatus.getPath().toString() + " - " + " execute allowed for current process");    	
    	return hasExcute;
    }
